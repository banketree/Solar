...
public class LineChart extends XYChart
{
		...
		for (FillOutsideLine fill : fillOutsideLine)
		{
			if (fill != FillOutsideLine.NONE)
			{
				...
				if (fill == FillOutsideLine.BOUNDS_ABOVE || fill == FillOutsideLine.BOUNDS_BELOW)
				{
					...
					boolean add = false;
					if (fill == FillOutsideLine.BOUNDS_ABOVE && fillPoints.get(1) < referencePoint || fill == FillOutsideLine.BOUNDS_BELOW && fillPoints.get(1) > referencePoint)
					{
					...
					for (int i = 3; i < fillPoints.size(); i += 2)
					{
						...
						{
							if (add || fill == FillOutsideLine.BOUNDS_ABOVE && value < referencePoint || fill == FillOutsideLine.BOUNDS_BELOW && value > referencePoint)
							{
	...
	protected ClickableArea[] clickableAreasForPoints(List<Float> points, List<Double> values, float yAxisValue, int seriesIndex, int startIndex)
	{
		...
		for (int i = 0; i < length; i += 2)
		{
			...
			ret[i / 2] = new ClickableArea(new RectF(points.get(i) - selectableBuffer, points.get(i + 1) - selectableBuffer, points.get(i) + selectableBuffer, points.get(i + 1) + selectableBuffer), values.get(i), values.get(i + 1));
		}
		return ret;
	}
