...
public abstract class XYChart extends AbstractChart
{
	...
	protected void drawXLabels(List<Double> xLabels, Double[] xTextLabelLocations, Canvas canvas, Paint paint, int left, int top, int bottom, double xPixelsPerUnit, double minX, double maxX)
	{
		...
		for (int i = 0; i < length; i++)
		{
			...
			if (showLabels)
			{
				...
				drawText(canvas, getLabel(mRenderer.getLabelFormat(), label), xLabel, bottom + mRenderer.getLabelsTextSize() * 4 / 3 + mRenderer.getXLabelsPadding(), paint, mRenderer.getXLabelsAngle());
			}
			if (showGridY)
			{
				paint.setColor(mRenderer.getGridColor());
			...
			for (int j = 0; j < length; j++)
			{
				...
				} else if (or == Orientation.VERTICAL)
				{
					if (showLabels && !textLabel)
					{
						paint.setColor(mRenderer.getYLabelsColor(i));
						canvas.drawLine(right - getLabelLinePos(axisAlign), yLabel, right, yLabel, paint);
						drawText(canvas, getLabel(mRenderer.getLabelFormat(), label), right + 10 + mRenderer.getYLabelsPadding(), yLabel - 2, paint, mRenderer.getYLabelsAngle());
					}
					if (showGridX)
					{
						paint.setColor(mRenderer.getGridColor());
		...
		if (showLabels)
		{
			...
			for (Double location : xTextLabelLocations)
			{
				if (minX <= location && location <= maxX)
				{
					...
					paint.setColor(mRenderer.getXLabelsColor());
					canvas.drawLine(xLabel, bottom, xLabel, bottom + mRenderer.getLabelsTextSize() / 3, paint);
					drawText(canvas, mRenderer.getXTextLabel(location), xLabel, bottom + mRenderer.getLabelsTextSize() * 4 / 3, paint, mRenderer.getXLabelsAngle());
					if (showCustomTextGrid)
					{
						paint.setColor(mRenderer.getGridColor());
						canvas.drawLine(xLabel, bottom, xLabel, top, paint);
	...
	public double[] toRealPoint(float screenX, float screenY, int scale)
	{
		...
		{
			return new double[]
			{ (screenX - mScreenR.left) * (realMaxX - realMinX) / mScreenR.width() + realMinX, (mScreenR.top + mScreenR.height() - screenY) * (realMaxY - realMinY) / mScreenR.height() + realMinY };
		} else
		{
			return new double[]
			{ screenX, screenY };
	...
	public double[] toScreenPoint(double[] realPoint, int scale)
	{
		...
		{
			return new double[]
			{ (realPoint[0] - realMinX) * mScreenR.width() / (realMaxX - realMinX) + mScreenR.left, (realMaxY - realPoint[1]) * mScreenR.height() / (realMaxY - realMinY) + mScreenR.top };
		} else
		{
			return realPoint;
		}
	 ...
	 *            the start index of the rendering points
	 */
	public abstract void drawSeries(Canvas canvas, Paint paint, List<Float> points, SimpleSeriesRenderer seriesRenderer, float yAxisValue, int seriesIndex, int startIndex);
